...
const getPositionEventString = function(e) {

  let $el = $(e.target); // clicked element

  // get first part
  let result = $('body').data('tracking-data') + '/';

  // get second part
  if ($el.parents('#sidebar').length) {
    result += 'SB/';
  } else if ($el.parents('#header').length || $el.parents('.shop-header-wrapper').length) {
    result += 'HD/';
  } else if ($el.attr('name') === 'query' || $el.parents('#search-header-results').length) {
    result += 'SE/';
  } else {
    result += 'MB/';
  }
  ...
  if ($el.parents('.card-coupons-list').length) {
    result += 'CL/';
    result += $el.closest('li.item').data('index');
    // Related Coupons Widget
  } else if ($el.parents('.box-related-coupons').length) {
    result += 'RCW/';
    result += $el.closest('li').data('index');
    // Top Coupons Widget
  } else if ($el.parents('.top-x-coupons').length) {
    result += 'TCW/';
    result += $el.closest('li').index();
    // Fallback Link
  } else if ($el.hasClass('fallback_link')) {
    result += 'FU/';
    result += 'null/';
    // Shop Header Logo
  } else if ($el.parents('.shop-header-logo').length) {
    result += 'SPL/';
    result += 'null/';
    // Content Text Image and Link
  } else if ($el.parents('.card-about-this-shop').length ||
    $el.parents('.card-about-this-category').length ||
    $el.parents('.card-about-this-campaign').length) {

    // Image Clicks
    if (typeof $el.attr('src') !== 'undefined' || $el.children('img').length) {
      result += 'ICT/';
    } else {
      // Link Clicks
      result += 'CT/';
    ...
    result += 'null/';
    // Coupons List Widget
  } else if ($el.parents('.card-coupons-lists').length) {
    let lis = $el.closest('.card-coupons-lists').find('.tab-pane li').not('.text-center');
    result += 'CLW/';
    result += lis.index($el.closest('li'));
  } else if ($el.attr('name') === 'query' || $el.parents('#search-header-results').length) {
    result += 'SR/';
    if ($el.attr('name') === 'query') {
      // clickout with the keyboard
      result += $('#search-header-results ul li.active').index();
    } else {
      // clickout with the mouse
      result += $el.parents('li').index();
    }
  } else {
    result += 'null/';
    result += 'null/';
...
const add_dataLayer_event = function(ele, type) {
...
const init_element_tracking = function(element) {
...
const get_card_title = function(element) {
  switch (true) {
    case $(element).hasClass('card-filter-by-letter'):
      return 'box-filter-by-letter';
    case $(element).hasClass('top-x-coupons'):
      return 'box-top-coupons';
    case $(element).hasClass('card-categories-index'):
      return 'box-categories';
    case $(element).hasClass('additional-info'):
      return 'box-shopinfo';
    case $(element).hasClass('card-social-media'):
      return 'box-social';
    case $(element).hasClass('card-shops-list'):
      return 'box-shops-list';
    case $(element).hasClass('card-list-coupon-type'):
      return 'box-filter-by-type';
    case $(element).hasClass('card-text'):
      return 'box-text';
    case $(element).hasClass('card-image'):
      return 'ad-image';
    case $(element).hasClass('card-see-more-categories'):
      return 'box-categories';
    case $(element).hasClass('card-newsletter'):
      return 'box-nl-with-field';
    default:
      return 'card';
  }
};

const add_event_action = function(element, string) {
  $(element).attr('data-event-action', string);
};

const add_event_label = function(element, string) {
  $(element).attr('data-event-label', string);
};

const add_event_category = function(element, string) {
  $(element).attr('data-event-category', string);
};

const set_element_tracking_data = function() {
  ...
  // featured shops
  let featured_shops = $('.featured-shops a');
  featured_shops.each(function(i) {
    let val = i + 1 === featured_shops.length ? 'showall' : $(featured_shops).index($(this)) + '_' + $(this).data('shop-name');
  ...
  let sidebar_cards = $('#sidebar .card');
  ...
  let top_x_coupons_links = $('.top-x-coupons a');
  ...
  $('.card-coupons-lists .tab-content a').each(function(i) {
    let index = $('.card-coupons-lists .tab-content li').index($(this).closest('li'));
    add_event_action($(this), (index !== -1 ? index + '_' : '') + $(this).find('span').text().trim());
  });

  // content coupons-list
  let coupons_list_lis = $('.card-coupons-list li.item');
  ...
  let related_shops_li = $('.related-shops li');
  ...
  $(document).on('click', '.card-about-this-shop .card-content a, .card-about-this-shop .card-content img', function(e) {
    ...
    dataLayer.push({
      'event': 'clickout',
      'eventCategory': 'Clickout - ' + $('body').data('tracking-data').replace('Shop%20-%20', ''),
      'eventAction': decodeURIComponent(getPositionEventString(e)) + 'null/' + page,
      'eventLabel': 'null',
      'eventValue': '0'
    });
...
$(document).ready(set_element_tracking_data);

export {getPositionEventString, init_element_tracking, add_event_category}
