var pc_tracking_user_id, ga_uid, geo_ip_country;

var block_votes_if_voted = function() {
  $('.star-rating.voted>ul').css('pointer-events', 'none')
    .find('.btn')
    .css('opacity', '0.6');
};

var block_and_recalculate_votes = function(star) {
  $('.star-rating .list-inline.list-unstyled').hide();
  $review_count = $('.review-count');

  var total_reviews = parseInt($review_count.data('reviewcount')) + 1;
  var total_stars = parseInt($('.star-rating').data('total-stars')) + star;
  var total_rating = (total_stars / total_reviews).toFixed(2);

  $review_count.attr('data-reviewcount', total_reviews);
  $('.rating-value').attr('data-ratingvalue', total_rating.toString().replace(',', $review_count.data('separator')));
...
$.fn.star_rating = function(options, callback) {
  ...
  var $loaders = $('.shop-rating .loader');
